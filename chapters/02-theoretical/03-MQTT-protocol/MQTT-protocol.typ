...
- *Broker:* Centralny serwer odpowiedzialny za odbieranie wszystkich wiadomości, a następnie ich filtrowanie oraz dystrybucję do odpowiednich odbiorców. W Home Assistant najczęściej wykorzystywanym brokerem jest *Eclipse Mosquitto*.
...
Zasada działania polega na tym, że klient subskrybujący informuje brokera o chęci otrzymywania wiadomości z określonych tematów. Gdy inny klient (publikujący) wyśle wiadomość na dany temat, broker natychmiast przekazuje ją do wszystkich klientów, którzy subskrybują ten temat.

=== Mechanizmy zapewnienia jakości usług (QoS)

Jedną z cech MQTT jest możliwość definiowania poziomu jakości usług (ang. *Quality of Service* – QoS), co pozwala na dostosowanie niezawodności dostarczania wiadomości do wymagań konkretnego zastosowania:
...
- *Last Will and Testament (LWT):* Pozwala klientowi zdefiniować wiadomość, która zostanie automatycznie opublikowana przez brokera w przypadku nieoczekiwanego rozłączenia klienta (np. utraty zasilania lub zasięgu Wi-Fi). W Home Assistant mechanizm ten jest kluczowy dla oznaczania urządzeń jako niedostępne.
...
Bardzo pomocnym może być mechanizm *MQTT Discovery*. Pozwala on urządzeniom na automatyczne przedstawienie się systemowi Home Assistant poprzez wysłanie specjalnie sformatowanej wiadomości konfiguracyjnej w formacie JSON na określony temat (domyślnie `homeassistant/`). Dzięki temu użytkownik nie musi ręcznie edytować plików konfiguracyjnych – nowe urządzenie pojawia się w interfejsie automatycznie jako gotowa do użycia encja.
