...
A key element of the thesis is the security research of the deployed system. These studies were conducted on a prepared test bench where the IoT network was physically and logically isolated from the home network using a TP-Link router. Tests at the network layer allowed for the verification of the effectiveness of ICMP traffic isolation under three different configuration scenarios. In turn, application layer tests confirmed that the MQTT password authentication requirement effectively blocks unauthorized access to communication with the broker. Secure remote access to the Home Assistant interface was also investigated using the Tailscale VPN network, which operates on the WireGuard protocol.
